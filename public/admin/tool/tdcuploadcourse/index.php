...
$returnurl = new moodle_url('/admin/tool/tdcuploadcourse/index.php');
...
if ($categoryid) {
    ...
    $PAGE->set_url(new moodle_url('/admin/tool/tdcuploadcourse/index.php', ['categoryid' => $categoryid]));
    $PAGE->set_pagelayout('admin');
    $PAGE->set_title("$categoryname: " . get_string('uploadcourses', 'tool_tdcuploadcourse'));
    $PAGE->set_heading($categoryname);
} else {
    admin_externalpage_setup('tooltdcuploadcourse');
}

if (empty($importid)) {
    $mform1 = new tool_tdcuploadcourse_step1_form();
    ...
    if ($form1data = $mform1->get_data()) {
        ...
        unset($content);
        if ($readcount === false) {
            throw new \moodle_exception('csvfileerror', 'tool_tdcuploadcourse', $returnurl, $cir->get_error());
        } else if ($readcount == 0) {
            throw new \moodle_exception('csvemptyfile', 'error', $returnurl, $cir->get_error());
        }
    } else {
        echo $OUTPUT->header();
        echo $OUTPUT->heading_with_help(get_string('uploadcourses', 'tool_tdcuploadcourse'), 'uploadcourses', 'tool_tdcuploadcourse');
        $mform1->display();
        echo $OUTPUT->footer();
        die();
    }
...
$mform2 = new tool_tdcuploadcourse_step2_form(null, array('contextid' => $context->id, 'columns' => $cir->get_columns(),
    'data' => $data));
...
if ($form2data = $mform2->is_cancelled()) {
    ...
    $customfields = tool_tdcuploadcourse_helper::get_custom_course_field_names();
    ...
    $processor = new tool_tdcuploadcourse_processor($cir, $options, $defaults);

    echo $OUTPUT->header();
    if (isset($form2data->showpreview)) {
        echo $OUTPUT->heading(get_string('uploadcoursespreview', 'tool_tdcuploadcourse'));
        $processor->preview($previewrows, new tool_tdcuploadcourse_tracker(tool_tdcuploadcourse_tracker::OUTPUT_HTML));
        $mform2->display();
    } else {
        echo $OUTPUT->heading(get_string('uploadcoursesresult', 'tool_tdcuploadcourse'));
        $processor->execute(new tool_tdcuploadcourse_tracker(tool_tdcuploadcourse_tracker::OUTPUT_HTML));
    ...
    if (!empty($form1data)) {
        ...
        $options = array('mode' => tool_tdcuploadcourse_processor::MODE_CREATE_NEW);
    }
    $processor = new tool_tdcuploadcourse_processor($cir, $options, $categorydefaults);
    echo $OUTPUT->header();
    echo $OUTPUT->heading(get_string('uploadcoursespreview', 'tool_tdcuploadcourse'));
    $processor->preview($previewrows, new tool_tdcuploadcourse_tracker(tool_tdcuploadcourse_tracker::OUTPUT_HTML));
    $mform2->display();
