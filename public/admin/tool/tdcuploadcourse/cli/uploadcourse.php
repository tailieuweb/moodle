...
$modes = array(
    'createnew' => tool_tdcuploadcourse_processor::MODE_CREATE_NEW,
    'createall' => tool_tdcuploadcourse_processor::MODE_CREATE_ALL,
    'createorupdate' => tool_tdcuploadcourse_processor::MODE_CREATE_OR_UPDATE,
    'update' => tool_tdcuploadcourse_processor::MODE_UPDATE_ONLY
);
if (!isset($options['mode']) || !isset($modes[$options['mode']])) {
    echo get_string('invalidmode', 'tool_tdcuploadcourse')."\n";
    echo $help;
    die();
}
...
$updatemodes = array(
    'nothing' => tool_tdcuploadcourse_processor::UPDATE_NOTHING,
    'dataonly' => tool_tdcuploadcourse_processor::UPDATE_ALL_WITH_DATA_ONLY,
    'dataordefaults' => tool_tdcuploadcourse_processor::UPDATE_ALL_WITH_DATA_OR_DEFAUTLS,
    'missingonly' => tool_tdcuploadcourse_processor::UPDATE_MISSING_WITH_DATA_OR_DEFAUTLS
);
if (($processoroptions['mode'] === tool_tdcuploadcourse_processor::MODE_CREATE_OR_UPDATE ||
        $processoroptions['mode'] === tool_tdcuploadcourse_processor::MODE_UPDATE_ONLY)
        && (!isset($options['updatemode']) || !isset($updatemodes[$options['updatemode']]))) {
    echo get_string('invalideupdatemode', 'tool_tdcuploadcourse')."\n";
    echo $help;
    die();
}
...
if (!file_exists($options['file'])) {
    echo get_string('invalidcsvfile', 'tool_tdcuploadcourse')."\n";
    echo $help;
    die();
}
...
if (!isset($encodings[$options['encoding']])) {
    echo get_string('invalidencoding', 'tool_tdcuploadcourse')."\n";
    echo $help;
    die();
}
...
if ($options['restorefile'] && !file_exists($options['restorefile'])) {
    echo get_string('invalidrestorefile', 'tool_tdcuploadcourse')."\n";
    echo $help;
    die();
}
...
unset($content);
if ($readcount === false) {
    throw new \moodle_exception('csvfileerror', 'tool_tdcuploadcourse', '', $cir->get_error());
} else if ($readcount == 0) {
    throw new \moodle_exception('csvemptyfile', 'error', '', $cir->get_error());
}
$processor = new tool_tdcuploadcourse_processor($cir, $processoroptions, $defaults);
$processor->execute(new tool_tdcuploadcourse_tracker(tool_tdcuploadcourse_tracker::OUTPUT_PLAIN));
