 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class tool_tdcuploadcourse_step2_form extends tool_tdcuploadcourse_base_form {
    ...
    public function definition() {
        ...
        // Course options.
        $mform->addElement('header', 'courseoptionshdr', get_string('courseprocess', 'tool_tdcuploadcourse'));
        $mform->setExpanded('courseoptionshdr', true);

        $mform->addElement('text', 'options[shortnametemplate]', get_string('shortnametemplate', 'tool_tdcuploadcourse'),
            'maxlength="100" size="20"');
        $mform->setType('options[shortnametemplate]', PARAM_RAW);
        $mform->addHelpButton('options[shortnametemplate]', 'shortnametemplate', 'tool_tdcuploadcourse');
        $mform->hideIf('options[shortnametemplate]', 'options[mode]', 'eq', tool_tdcuploadcourse_processor::MODE_CREATE_OR_UPDATE);
        $mform->hideIf('options[shortnametemplate]', 'options[mode]', 'eq', tool_tdcuploadcourse_processor::MODE_UPDATE_ONLY);

        // Restore file is not in the array options on purpose, because formslib can't handle it!
        $contextid = $this->_customdata['contextid'];
        $mform->addElement('hidden', 'contextid', $contextid);
        $mform->setType('contextid', PARAM_INT);
        $mform->addElement('filepicker', 'restorefile', get_string('templatefile', 'tool_tdcuploadcourse'));
        $mform->addHelpButton('restorefile', 'templatefile', 'tool_tdcuploadcourse');

        $mform->addElement('text', 'options[templatecourse]', get_string('coursetemplatename', 'tool_tdcuploadcourse'));
        $mform->setType('options[templatecourse]', PARAM_TEXT);
        $mform->addHelpButton('options[templatecourse]', 'coursetemplatename', 'tool_tdcuploadcourse');

        $mform->addElement('selectyesno', 'options[reset]', get_string('reset', 'tool_tdcuploadcourse'));
        $mform->setDefault('options[reset]', 0);
        $mform->hideIf('options[reset]', 'options[mode]', 'eq', tool_tdcuploadcourse_processor::MODE_CREATE_NEW);
        $mform->hideIf('options[reset]', 'options[mode]', 'eq', tool_tdcuploadcourse_processor::MODE_CREATE_ALL);
        $mform->disabledIf('options[reset]', 'options[allowresets]', 'eq', 0);
        $mform->addHelpButton('options[reset]', 'reset', 'tool_tdcuploadcourse');

        // Default values.
        $mform->addElement('header', 'defaultheader', get_string('defaultvalues', 'tool_tdcuploadcourse'));
        ...
        $handler->instance_form_definition($mform, 0, 'defaultvaluescustomfieldcategory', 'tool_tdcuploadcourse');
        ...
        $this->add_action_buttons(true, get_string('uploadcourses', 'tool_tdcuploadcourse'));
    ...
    public function add_action_buttons($cancel = true, $submitlabel = null) {
        $mform =& $this->_form;
        $buttonarray = array();
        $buttonarray[] = &$mform->createElement('submit', 'showpreview', get_string('preview', 'tool_tdcuploadcourse'));
