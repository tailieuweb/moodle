 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class tool_tdcuploadcourse_helper {
    ...
    public static function generate_shortname($data, $templateshortname) {
        ...
        $callback = partial(array('tool_tdcuploadcourse_helper', 'generate_shortname_callback'), $fullname, $idnumber);
    ...
    public static function get_restore_content_dir($backupfile = null, $shortname = null, &$errors = array()) {
        ...
        if (!empty($backupfile)) {
            ...
            if (empty($backupfile) || !is_readable($backupfile)) {
                $errors['cannotreadbackupfile'] = new lang_string('cannotreadbackupfile', 'tool_tdcuploadcourse');
                return false;
            }
        ...
        if ($usecache) {
            $cache = cache::make('tool_tdcuploadcourse', 'helper');
        }
        ...
        if (!$usecache || (($backupid = $cache->get($cachekey)) === false || !is_dir(get_backup_temp_directory($backupid)))) {
            ...
            if (!empty($backupfile)) {
                ...
                if (!$result) {
                    $errors['invalidbackupfile'] = new lang_string('invalidbackupfile', 'tool_tdcuploadcourse');
                }
            } else if (!empty($shortname) || is_numeric($shortname)) {
                ...
                if (!empty($courseid)) {
                    ...
                    $errors['coursetorestorefromdoesnotexist'] =
                        new lang_string('coursetorestorefromdoesnotexist', 'tool_tdcuploadcourse');
                }
            }
    ...
    public static function get_role_names($data, &$errors = array()) {
        ...
        if (!empty($invalidroles)) {
            $errors['invalidroles'] = new lang_string('invalidroles', 'tool_tdcuploadcourse', implode(', ', $invalidroles));
        }
        ...
        foreach ($data as $name => $originalvalue) {
            if (preg_match('/^customfield_(?<name>.*)?$/', $name, $matches)
                ...
                if (count($validationerrors) > 0) {
                    $errors['customfieldinvalid'] = new lang_string('customfieldinvalid', 'tool_tdcuploadcourse',
        ...
        if (!empty($data['category'])) {
            ...
            if (!empty($category) && !empty($category->id)) {
                ...
                $errors['couldnotresolvecatgorybyid'] =
                    new lang_string('couldnotresolvecatgorybyid', 'tool_tdcuploadcourse');
            }
        }
        ...
        if (empty($catid) && !empty($data['category_idnumber'])) {
            ...
            if (empty($catid)) {
                $errors['couldnotresolvecatgorybyidnumber'] =
                    new lang_string('couldnotresolvecatgorybyidnumber', 'tool_tdcuploadcourse');
            }
        }
        if (empty($catid) && !empty($data['category_path'])) {
            ...
            if (empty($catid)) {
                $errors['couldnotresolvecatgorybypath'] =
                    new lang_string('couldnotresolvecatgorybypath', 'tool_tdcuploadcourse');
            }
        }
