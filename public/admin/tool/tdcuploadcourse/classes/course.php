 ...
 *
 * @package    tool_tdcuploadcourse
 * @copyright  2013 Frédéric Massart
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

use tool_tdcuploadcourse\permissions;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class tool_tdcuploadcourse_course {
    ...
    /** @var int import mode. Matches tool_tdcuploadcourse_processor::MODE_* */
    protected $mode;
    ...
    /** @var int update mode. Matches tool_tdcuploadcourse_processor::UPDATE_* */
     ...
     * Constructor
     *
     * @param int $mode import mode, constant matching tool_tdcuploadcourse_processor::MODE_*
     * @param int $updatemode update mode, constant matching tool_tdcuploadcourse_processor::UPDATE_*
    ...
    public function __construct($mode, $updatemode, $rawdata, $defaults = array(), $importoptions = array()) {
        ...
        if ($mode !== tool_tdcuploadcourse_processor::MODE_CREATE_NEW &&
                $mode !== tool_tdcuploadcourse_processor::MODE_CREATE_ALL &&
                $mode !== tool_tdcuploadcourse_processor::MODE_CREATE_OR_UPDATE &&
                $mode !== tool_tdcuploadcourse_processor::MODE_UPDATE_ONLY) {
            throw new coding_exception('Incorrect mode.');
        } else if ($updatemode !== tool_tdcuploadcourse_processor::UPDATE_NOTHING &&
                $updatemode !== tool_tdcuploadcourse_processor::UPDATE_ALL_WITH_DATA_ONLY &&
                $updatemode !== tool_tdcuploadcourse_processor::UPDATE_ALL_WITH_DATA_OR_DEFAUTLS &&
                $updatemode !== tool_tdcuploadcourse_processor::UPDATE_MISSING_WITH_DATA_OR_DEFAUTLS) {
    ...
    public function can_create() {
        return in_array($this->mode, array(tool_tdcuploadcourse_processor::MODE_CREATE_ALL,
            tool_tdcuploadcourse_processor::MODE_CREATE_NEW,
            tool_tdcuploadcourse_processor::MODE_CREATE_OR_UPDATE)
        );
    }
    ...
    public function can_update() {
        return in_array($this->mode,
                array(
                    tool_tdcuploadcourse_processor::MODE_UPDATE_ONLY,
                    tool_tdcuploadcourse_processor::MODE_CREATE_OR_UPDATE)
                ) && $this->updatemode != tool_tdcuploadcourse_processor::UPDATE_NOTHING;
    }
    ...
    public function can_use_defaults() {
        return in_array($this->updatemode, array(tool_tdcuploadcourse_processor::UPDATE_MISSING_WITH_DATA_OR_DEFAUTLS,
            tool_tdcuploadcourse_processor::UPDATE_ALL_WITH_DATA_OR_DEFAUTLS));
    }
    ...
    protected function get_valid_fields() {
        return array_merge(self::$validfields, \tool_tdcuploadcourse_helper::get_custom_course_field_names());
    }
    ...
    protected function get_restore_content_dir() {
        ...
        $errors = array();
        $dir = tool_tdcuploadcourse_helper::get_restore_content_dir($backupfile, $shortname, $errors);
        if (!empty($errors)) {
    ...
    public function prepare() {
        ...
        if (!empty($this->shortname) || is_numeric($this->shortname)) {
            if ($this->shortname !== clean_param($this->shortname, PARAM_TEXT)) {
                $this->error('invalidshortname', new lang_string('invalidshortname', 'tool_tdcuploadcourse'));
                return false;
            }

            // Ensure we don't overflow the maximum length of the shortname field.
            if (core_text::strlen($this->shortname) > 255) {
                $this->error('invalidshortnametoolong', new lang_string('invalidshortnametoolong', 'tool_tdcuploadcourse', 255));
                return false;
            }
        }
        ...
        if ($this->options['delete']) {
            if (!$exists) {
                $this->error('cannotdeletecoursenotexist', new lang_string('cannotdeletecoursenotexist', 'tool_tdcuploadcourse'));
                return false;
            } else if (!$this->can_delete()) {
                $this->error('coursedeletionnotallowed', new lang_string('coursedeletionnotallowed', 'tool_tdcuploadcourse'));
                return false;
            }
        ...
        if ($exists) {
            if ($this->mode === tool_tdcuploadcourse_processor::MODE_CREATE_NEW) {
                $this->error('courseexistsanduploadnotallowed',
                    new lang_string('courseexistsanduploadnotallowed', 'tool_tdcuploadcourse'));
                return false;
            } else if ($this->can_update()) {
                // We can never allow for any front page changes!
                if ($this->shortname == $SITE->shortname) {
                    $this->error('cannotupdatefrontpage', new lang_string('cannotupdatefrontpage', 'tool_tdcuploadcourse'));
                    return false;
                }
            }
        } else {
            if (!$this->can_create()) {
                $this->error('coursedoesnotexistandcreatenotallowed',
                    new lang_string('coursedoesnotexistandcreatenotallowed', 'tool_tdcuploadcourse'));
                return false;
            }
        }
        ...
        $errors = array();
        $catid = tool_tdcuploadcourse_helper::resolve_category($this->rawdata, $errors);
        if (empty($errors)) {
        ...
        if (
            ...
            $this->error('invalidfullnametoolong', new lang_string('invalidfullnametoolong', 'tool_tdcuploadcourse',
                \core_course\constants::FULLNAME_MAXIMUM_LENGTH));
            return false;
        }

        // If the course does not exist, or will be forced created.
        if (!$exists || $mode === tool_tdcuploadcourse_processor::MODE_CREATE_ALL) {
            ...
            if (!empty($errors)) {
                $this->error('missingmandatoryfields', new lang_string('missingmandatoryfields', 'tool_tdcuploadcourse',
                    implode(', ', $errors)));
                return false;
            }
        }
        ...
        if (!empty($this->options['rename']) || is_numeric($this->options['rename'])) {
            if (!$this->can_update()) {
                $this->error('canonlyrenameinupdatemode', new lang_string('canonlyrenameinupdatemode', 'tool_tdcuploadcourse'));
                return false;
            } else if (!$exists) {
                $this->error('cannotrenamecoursenotexist', new lang_string('cannotrenamecoursenotexist', 'tool_tdcuploadcourse'));
                return false;
            } else if (!$this->can_rename()) {
                $this->error('courserenamingnotallowed', new lang_string('courserenamingnotallowed', 'tool_tdcuploadcourse'));
                return false;
                ...
                $this->error('cannotrenameshortnamealreadyinuse',
                    new lang_string('cannotrenameshortnamealreadyinuse', 'tool_tdcuploadcourse'));
                return false;
                ...
                $this->error('cannotrenameidnumberconflict', new lang_string('cannotrenameidnumberconflict', 'tool_tdcuploadcourse'));
                return false;
            }
            $coursedata['shortname'] = $this->options['rename'];
            $this->status('courserenamed', new lang_string('courserenamed', 'tool_tdcuploadcourse',
        ...
        if (empty($this->shortname) && !is_numeric($this->shortname)) {
            if (empty($this->importoptions['shortnametemplate'])) {
                $this->error('missingshortnamenotemplate', new lang_string('missingshortnamenotemplate', 'tool_tdcuploadcourse'));
                return false;
            } else if (!$this->can_only_create()) {
                $this->error('cannotgenerateshortnameupdatemode',
                    new lang_string('cannotgenerateshortnameupdatemode', 'tool_tdcuploadcourse'));
                return false;
            } else {
                $newshortname = tool_tdcuploadcourse_helper::generate_shortname($coursedata,
                    $this->importoptions['shortnametemplate']);
                if (is_null($newshortname)) {
                    $this->error('generatedshortnameinvalid', new lang_string('generatedshortnameinvalid', 'tool_tdcuploadcourse'));
                    return false;
                } else if ($this->exists($newshortname)) {
                    if ($mode === tool_tdcuploadcourse_processor::MODE_CREATE_NEW) {
                        $this->error('generatedshortnamealreadyinuse',
                            new lang_string('generatedshortnamealreadyinuse', 'tool_tdcuploadcourse'));
                        return false;
                    }
                    $exists = true;
                }
                $this->status('courseshortnamegenerated', new lang_string('courseshortnamegenerated', 'tool_tdcuploadcourse',
                    $newshortname));
        ...
        if ($exists && $mode === tool_tdcuploadcourse_processor::MODE_CREATE_ALL) {
            ...
            $this->shortname = tool_tdcuploadcourse_helper::increment_shortname($this->shortname);
            $exists = false;
            if ($this->shortname != $original) {
                $this->status('courseshortnameincremented', new lang_string('courseshortnameincremented', 'tool_tdcuploadcourse',
                    array('from' => $original, 'to' => $this->shortname)));
                if (isset($coursedata['idnumber'])) {
                    $originalidn = $coursedata['idnumber'];
                    $coursedata['idnumber'] = tool_tdcuploadcourse_helper::increment_idnumber($coursedata['idnumber']);
                    if ($originalidn != $coursedata['idnumber']) {
                        $this->status('courseidnumberincremented', new lang_string('courseidnumberincremented', 'tool_tdcuploadcourse',
        ...
        if (!$exists && isset($coursedata['idnumber'])) {
            if ($DB->count_records_select('course', 'idnumber = :idn', array('idn' => $coursedata['idnumber'])) > 0) {
                $this->error('idnumberalreadyinuse', new lang_string('idnumberalreadyinuse', 'tool_tdcuploadcourse'));
                return false;
            }
        }
        ...
        if (!empty($coursedata['lang'])) {
            if ($exists) {
                ...
                if (!has_capability('moodle/course:setforcedlanguage', context_course::instance($courseid))) {
                    $this->error('cannotforcelang', new lang_string('cannotforcelang', 'tool_tdcuploadcourse'));
                    return false;
                }
            } else {
        ...
        switch ($mode) {
            case tool_tdcuploadcourse_processor::MODE_CREATE_NEW:
            case tool_tdcuploadcourse_processor::MODE_CREATE_ALL:
                if ($exists) {
                    $this->error('courseexistsanduploadnotallowed',
                        new lang_string('courseexistsanduploadnotallowed', 'tool_tdcuploadcourse'));
                    return false;
                ...
                break;
            case tool_tdcuploadcourse_processor::MODE_UPDATE_ONLY:
                if (!$exists) {
                    $this->error('coursedoesnotexistandcreatenotallowed',
                        new lang_string('coursedoesnotexistandcreatenotallowed', 'tool_tdcuploadcourse'));
                    return false;
                ...
                // No break!
            case tool_tdcuploadcourse_processor::MODE_CREATE_OR_UPDATE:
                if ($exists) {
                    if ($updatemode === tool_tdcuploadcourse_processor::UPDATE_NOTHING) {
                        $this->error('updatemodedoessettonothing',
                            new lang_string('updatemodedoessettonothing', 'tool_tdcuploadcourse'));
                        return false;
                    }
                }
                break;
            default:
                ...
                $this->error('unknownimportmode', new lang_string('unknownimportmode', 'tool_tdcuploadcourse'));
                return false;
        }

        // Get final data.
        if ($exists) {
            $missingonly = ($updatemode === tool_tdcuploadcourse_processor::UPDATE_MISSING_WITH_DATA_OR_DEFAUTLS);
        ...
        // Add role renaming.
        $errors = array();
        $rolenames = tool_tdcuploadcourse_helper::get_role_names($this->rawdata, $errors);
        if (!empty($errors)) {
            foreach ($errors as $key => $message) {
                $this->error($key, $message);
            }
        ...
        $customfielddata = tool_tdcuploadcourse_helper::get_custom_course_field_data($this->rawdata, $this->defaults, $context,
            $errors);
        if (!empty($errors)) {
            foreach ($errors as $key => $message) {
                $this->error($key, $message);
        ...
        // Some validation.
        if (!empty($coursedata['format']) && !in_array($coursedata['format'], tool_tdcuploadcourse_helper::get_course_formats())) {
            $this->error('invalidcourseformat', new lang_string('invalidcourseformat', 'tool_tdcuploadcourse'));
            return false;
        }
        ...
        if (!$exists || !array_key_exists('numsections', $coursedata)) {
            if (isset($this->rawdata['numsections']) && is_numeric($this->rawdata['numsections'])) {
                ...
                $numsections = tool_tdcuploadcourse_helper::get_coursesection_count($this->options['templatecourse']);
                if ($numsections != 0) {
        ...
        if (!empty($coursedata['visible']) AND !($coursedata['visible'] == 0 OR $coursedata['visible'] == 1)) {
            $this->error('invalidvisibilitymode', new lang_string('invalidvisibilitymode', 'tool_tdcuploadcourse'));
            return false;
        }
        ...
        if (isset($coursedata['downloadcontent'])) {
            if (!$CFG->downloadcoursecontentallowed ||
                ...
                $this->error('downloadcontentnotallowed', new lang_string('downloadcontentnotallowed', 'tool_tdcuploadcourse'));
                return false;
            }
            ...
            if (!in_array($coursedata['downloadcontent'], $downloadcontentvalues)) {
                $this->error('invaliddownloadcontent', new lang_string('invaliddownloadcontent', 'tool_tdcuploadcourse'));
                return false;
            }
        }
        ...
        $this->enrolmentdata = tool_tdcuploadcourse_helper::get_enrolment_data($this->rawdata);
        ...
        if ($this->importoptions['reset'] || $this->options['reset']) {
            if ($this->do !== self::DO_UPDATE) {
                $this->error('canonlyresetcourseinupdatemode',
                    new lang_string('canonlyresetcourseinupdatemode', 'tool_tdcuploadcourse'));
                return false;
            } else if (!$this->can_reset()) {
                $this->error('courseresetnotallowed', new lang_string('courseresetnotallowed', 'tool_tdcuploadcourse'));
                return false;
            }
    ...
    public function proceed() {
        ...
        if ($this->do === self::DO_DELETE) {
            if ($this->delete()) {
                $this->status('coursedeleted', new lang_string('coursedeleted', 'tool_tdcuploadcourse'));
            } else {
                $this->error('errorwhiledeletingcourse', new lang_string('errorwhiledeletingcourse', 'tool_tdcuploadcourse'));
            }
            return true;
        } else if ($this->do === self::DO_CREATE) {
            $course = create_course((object) $this->data);
            $this->id = $course->id;
            $this->status('coursecreated', new lang_string('coursecreated', 'tool_tdcuploadcourse'));
        } else if ($this->do === self::DO_UPDATE) {
            $course = (object) $this->data;
            update_course($course);
            $this->id = $course->id;
            $this->status('courseupdated', new lang_string('courseupdated', 'tool_tdcuploadcourse'));
        } else {
        ...
        if (!empty($this->restoredata)) {
            ...
            if ($rc->execute_precheck()) {
                $rc->execute_plan();
                $this->status('courserestored', new lang_string('courserestored', 'tool_tdcuploadcourse'));
            } else {
                $this->error('errorwhilerestoringcourse', new lang_string('errorwhilerestoringcourse', 'tool_tdcuploadcourse'));
            }
            $rc->destroy();
        }
        ...
        if ($this->importoptions['reset'] || $this->options['reset']) {
            if ($this->do === self::DO_UPDATE && $this->can_reset()) {
                $this->reset($course);
                $this->status('coursereset', new lang_string('coursereset', 'tool_tdcuploadcourse'));
            }
        }
    ...
    protected function validate_enrolment_data(int $courseid, array $enrolmentdata): array {
        ...
        $enrolmentplugins = tool_tdcuploadcourse_helper::get_enrolment_plugins();
                ...
                if (!empty($options['delete'])) {
                    ...
                    foreach ($methodinstances as $methodinstance) {
                        if (!$plugin->can_delete_instance($methodinstance)) {
                            $errors['errorcannotdeleteenrolment'] = new lang_string('errorcannotdeleteenrolment',
                                'tool_tdcuploadcourse', $plugin->get_instance_name($methodinstance));
                            break;
                        }
                    }
                    ...
                    foreach ($methodinstances as $methodinstance) {
                        if (!$plugin->can_hide_show_instance($methodinstance)) {
                            $errors['errorcannotdisableenrolment'] =
                                new lang_string('errorcannotdisableenrolment', 'tool_tdcuploadcourse',
                    ...
                    if ((empty($methodinstance) && !$plugin->can_add_instance($courseid)) ||
                        ...
                        $errors['errorcannotcreateorupdateenrolment'] =
                            new lang_string('errorcannotcreateorupdateenrolment', 'tool_tdcuploadcourse',
                                $plugin->get_instance_name($methodinstance));
                ...
                if ($instance) {
                    ...
                    if ($plugin->can_delete_instance($instance) && $plugin->is_csv_upload_supported()) {
                        ...
                        $this->error('errorcannotdeleteenrolment',
                            new lang_string('errorcannotdeleteenrolment', 'tool_tdcuploadcourse',
    ...
    protected function reset($course) {
        ...
        // Add roles.
        $roles = tool_tdcuploadcourse_helper::get_role_ids();
