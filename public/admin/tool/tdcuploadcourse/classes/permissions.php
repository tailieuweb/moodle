...
class permissions {
    ...
    /**
     * Check permission to use tool_tdcuploadcourse in a given category.
     *
     * @param int $catid
    ...
    protected static function check_permission_to_use_uploadcourse_tool(
        ...
        if (!$category || !has_capability('tool/uploadcourse:use', $category->get_context())) {
            ...
            return new lang_string('courseuploadnotallowed', 'tool_tdcuploadcourse',
    ...
    /**
     * Check capabilities to delete a course and to use tool_tdcuploadcourse for it.
     *
     ...
     *
     * @param int $do one of tool_tdcuploadcourse_course::DO_UPDATE or tool_tdcuploadcourse_course::DO_ADD
    ...
    protected static function check_capability(int $do, array $coursedata, string $capability): ?lang_string {
        if ($do == tool_tdcuploadcourse_course::DO_UPDATE) {
     ...
     * - to use tool_tdcuploadcourse in the category where course is in and in the category where it will be moved to (if applicable).
    ...
    public static function check_permission_to_update(array $coursedata): ?lang_string {
        ...
        if ($error = self::check_permission_to_use_uploadcourse_tool($course->category,
                new lang_string('courseuploadupdatenotallowed', 'tool_tdcuploadcourse'))) {
            return $error;
        }
        ...
        if (!empty($coursedata['lang']) && $coursedata['lang'] !== $course->lang) {
            if (!has_capability('moodle/course:setforcedlanguage', $context)) {
                return new lang_string('cannotforcelang', 'tool_tdcuploadcourse');
            }
        }
        ...
        if (core_tag_tag::is_enabled('core', 'course') &&
            ...
            ($error = self::check_capability(tool_tdcuploadcourse_course::DO_UPDATE, $coursedata, 'moodle/course:tag'))) {
            return $error;
        }
     ...
     *
     * This checks capabilities:
     * - to use tool_tdcuploadcourse in the category where course will be created.
     * - to create a course.
     * - to force course language (if applicable).
    ...
    public static function check_permission_to_create(array $coursedata): ?lang_string {
        ...
        if (core_tag_tag::is_enabled('core', 'course') &&
                (array_key_exists('tags', $coursedata) && strval($coursedata['tags']) !== '') &&
                ($error = self::check_capability(tool_tdcuploadcourse_course::DO_CREATE, $coursedata, 'moodle/course:tag'))) {
            return $error;
        }
    ...
    public static function check_permission_to_reset(array $coursedata): ?lang_string {
        return self::check_capability(tool_tdcuploadcourse_course::DO_UPDATE, $coursedata, 'moodle/course:reset');
    }
